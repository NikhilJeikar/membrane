"""Command-line interface for shadow-pa."""
...
from shadow_pa.config import ensure_data_layout, get_settings, load_persona
from shadow_pa.inference.context import ContextBuilder
from shadow_pa.ingest.cursor import cursor_sessions_to_chat_sessions, ingest_cursor_path
from shadow_pa.ingest.lifecycle import parse_all_server_sources
from shadow_pa.ingest.server_models import SERVER_SOURCES
from shadow_pa.ingest.whatsapp import ingest_whatsapp_file
from shadow_pa.ingest.wikipedia import (
...
from shadow_pa.learning.chat_log import ChatLogger
from shadow_pa.learning.export import TrainingExporter
from shadow_pa.llm.ollama import OllamaClient
from shadow_pa.memory.models import MemoryCategory, ProposalStatus
from shadow_pa.memory.review import run_interactive_review
from shadow_pa.memory.store import MemoryStore
from shadow_pa.shadow.extractor import ShadowExtractor
from shadow_pa.shadow.offline import OfflineExtractor
from shadow_pa.server.app import run_server
from shadow_pa.server.auth import resolve_server_token
from shadow_pa.tracking.manifest import ManifestStore
from shadow_pa.utils.parallel import cpu_count, default_workers
from shadow_pa.utils.progress import make_rich_progress, noop_progress
...
def status_check() -> None:
    ...
    table = Table(title="shadow-pa status")
    ...
    table.add_row(
        "Cursor sessions parsed",
        str(len(list(settings.cursor_parsed_dir.glob("*.jsonl")))),
    )
    table.add_row(
    ...
    if not ollama_ok:
        console.print(
            "\n[yellow]Tip:[/yellow] Use `shadow-pa extract run --offline` or install Ollama:\n"
...
def tracking_reconcile(
    ...
    if source in ("cursor", "all"):
        total += manifest.reconcile_parsed_dir(
            "cursor", settings.cursor_raw_dir, settings.cursor_parsed_dir
        )
    ...
    if source not in ("cursor", "whatsapp", "email", "calendar", "search", "server", "all"):
        raise typer.BadParameter("source must be cursor, whatsapp, email, calendar, search, server, or all")
...
def tracking_mark_extracted(
    ...
    settings = _settings()
    manifest = _manifest()
    if source not in ("cursor", "whatsapp", "all"):
        raise typer.BadParameter("source must be cursor, whatsapp, or all")
    ...
    if source in ("cursor", "all"):
        count += len(
            [
                p
                for p in settings.cursor_parsed_dir.glob("*.jsonl")
    ...
    marked = 0
    if source in ("cursor", "all"):
        marked += manifest.mark_parsed_extracted("cursor", settings.cursor_parsed_dir)
    if source in ("whatsapp", "all"):
...
def server_status() -> None:
    ...
    table = Table(title="shadow-pa server ingest")
...
def ui_run(
    ...
    try:
        ...
        raise typer.Exit("Install UI deps: pip install 'shadow-pa[ui]'") from exc
    ...
    console.print(f"[green]shadow-pa UI[/green] → http://{host}:{port}")
...
def ingest_cursor(
    ...
    stats = ingest_cursor_path(
        path,
        raw_dir=settings.cursor_raw_dir,
        parsed_dir=settings.cursor_parsed_dir,
        redact=not no_redact,
        workers=n,
        manifest=_manifest(),
        force=force,
    ...
    if to_chats and stats.outputs:
        saved = cursor_sessions_to_chat_sessions(settings.cursor_parsed_dir, settings.chats_dir)
...
def extract_run(
    ...
    if source not in ("whatsapp", "cursor", "email", "calendar", "search", "server", "all"):
        raise typer.BadParameter(
            "source must be whatsapp, cursor, email, calendar, search, server, or all"
        )
    ...
    try:
        if use_offline:
            ...
            if source in ("cursor", "all"):
                stats = offline_extractor.extract_cursor_parsed_dir(
            ...
            if source in ("cursor", "all"):
                cursor_chunk = 20 if chunk_size == 40 else chunk_size
                stats = extractor.extract_cursor_parsed_dir(
                    settings.cursor_parsed_dir,
                    chunk_size=cursor_chunk,
                    user_only=self_only,
                    auto_propose=not dry_run,
                    workers=n_workers,
                    on_progress=on_progress,
    ...
    if total_proposals == 0:
        console.print(
            ...
            "Ingest data first: `shadow-pa ingest cursor ...` or `shadow-pa ingest whatsapp ...`"
        )
        return
...
def memory_list_proposed() -> None:
    ...
    console.print(table)
    console.print("\n[dim]Tip:[/dim] Run [bold]shadow-pa memory review[/bold] to approve/reject interactively.")
...
def dataset_prepare_summarization(
    ...
    if not raw_path.exists():
        raise typer.BadParameter(f"Corpus not found: {raw_path}. Run: shadow-pa ingest wiki")
...
def export_summarization(
    ...
    if not src.exists():
        raise typer.BadParameter(f"Dataset not found: {src}. Run: shadow-pa dataset prepare-summarization")
