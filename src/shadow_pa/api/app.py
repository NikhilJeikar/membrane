"""FastAPI control plane for shadow-pa UI."""
...
from shadow_pa.config import get_settings, load_persona
from shadow_pa.config_policy import TrainingPolicy, load_training_policy, save_training_policy
from shadow_pa.ingest.lifecycle import parse_all_server_sources, parse_server_source
...
from shadow_pa.memory.review import proposal_to_dict
...
def create_app(ui_dist: Path | None = None) -> FastAPI:
    app = FastAPI(title="shadow-pa", version="0.1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    def api_status() -> dict:
        ...
        return {
            ...
            "stale_extract_cursor": len(m.list_stale_extracted("cursor")),
            "cursor_parsed": len(list(settings.cursor_parsed_dir.glob("*.jsonl"))),
    ...
    def get_proposal(proposal_id: str) -> dict:
        ...
        from shadow_pa.memory.models import MemoryProposal
    ...
    def ingest_parse(body: ParseRequest) -> dict:
        ...
        if body.source == "whatsapp":
            raise HTTPException(400, "use CLI: shadow-pa ingest whatsapp")
