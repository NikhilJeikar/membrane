...
from shadow_pa.ingest.calendar import parse_calendar_raw
...
def clear_ingest_data(
    ...
    if source == "cursor":
        counts["raw"] = _unlink_files(settings.cursor_raw_dir, "*.jsonl")
        counts["parsed"] = _unlink_files(settings.cursor_parsed_dir)
        counts["chats"] = _unlink_files(settings.chats_dir, "cursor-*.json")
