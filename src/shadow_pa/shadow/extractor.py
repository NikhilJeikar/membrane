...
from shadow_pa.ingest.cursor import (
    chunk_cursor_turns,
...
from shadow_pa.ingest.whatsapp import (
    chunk_messages,
    format_chunk_for_extraction,
    load_parsed_messages,
)
from shadow_pa.llm.ollama import OllamaClient, OllamaError
...
class ShadowExtractor:
    ...
    def extract_cursor_parsed_dir(
        ...
        paths, skipped = self._select_parsed_files(
            parsed_dir, "cursor", manifest, only_new=only_new, force=force, skip_meta=True
        )
        tasks: list[ChunkTask] = []
        for path in paths:
            turns = load_parsed_cursor_turns(path)
            for chunk in chunk_cursor_turns(turns, chunk_size=chunk_size, user_only=user_only):
                tasks.append(
                    (
                        format_cursor_chunk_for_extraction(chunk),
                        MemorySource.CURSOR,
                        "Cursor",
                        path.name,
                    )
        ...
        proposals = self._extract_tasks(
            ...
            auto_propose=auto_propose,
            on_progress=on_progress,
            progress_label="Cursor chunks",
        )
        self._finalize_extract("cursor", paths, manifest, record=auto_propose)
        return ExtractStats(
            processed_files=len(paths),
            skipped_files=skipped,
            proposals=len(proposals),
