...
from membrane.ingest.server_common import save_raw_envelope
from membrane.ingest.server_models import SERVER_SOURCES, ServerSource
from membrane.server.auth import check_bearer_auth, resolve_server_token
from membrane.server.scheduler import ParseScheduler
...
class MembraneRequestHandler(BaseHTTPRequestHandler):
...
def run_server(
    ...
    *,
    ...
    handler = MembraneRequestHandler
    ...
    print(f"membrane server listening on http://{bind_host}:{bind_port}")
