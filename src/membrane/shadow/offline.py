...
from membrane.ingest.server_common import format_records_for_extraction, load_parsed_records
from membrane.ingest.whatsapp import load_parsed_messages
...
def offline_extract_agent_file(parsed_path: Path, parsed_dir: Path) -> list[MemoryProposal]:
    turns = load_parsed_turns(parsed_path)
    user_msgs = [t.content for t in turns if t.role == "user"]
    if not user_msgs:
        return []

    meta = read_session_meta(parsed_path)
    provider = meta.get("agent") or (turns[0].agent if turns else "agent")
    ...
    proposals: list[MemoryProposal] = []
    workspace = meta.get("workspace_hint")
    ...
    if workspace:
        proposals.append(
            MemoryProposal(
                category=MemoryCategory.PROFILE,
                source=mem_source,
                reason=f"Offline: {label} workspace",
                profile=ProfileEntry(
                    key=f"{provider}_project",
    ...
    combined = "\n".join(user_msgs)
    proposals.extend(_keyword_profiles(combined, mem_source))
    ...
    proposals.append(
        MemoryProposal(
            ...
            source=mem_source,
            reason=f"Offline: {label} session topics",
            episode=EpisodeEntry(
                summary=f"{label} session ({parsed_path.stem}): {topics}",
                tags=[provider, "dev", workspace or "general"],
                source=mem_source,
    ...
    if avg_len < 120:
        pref = f"writes short, direct prompts in {label}"
    elif avg_len > 400:
        pref = f"writes long, detailed prompts in {label}"
    else:
        pref = f"writes medium-length prompts in {label}"
...
def offline_extract_cursor_file(parsed_path: Path, parsed_dir: Path) -> list[MemoryProposal]:
    return offline_extract_agent_file(parsed_path, parsed_dir)
        ...
        *,
        ...
        for path in all_paths:
            ...
            if manifest and only_new and not force:
                if not manifest.needs_extract(provider, raw_key, path, force=False):
                    skipped += 1
                    continue
            paths.append(path)

        def extract_one(path: Path) -> list[MemoryProposal]:
            return offline_extract_agent_file(path, parsed_dir)

        try:
            progress_label = f"{get_adapter(provider).label} sessions"
        except ValueError:
            progress_label = f"{provider} sessions"
        ...
        if manifest and auto_propose:
            for path in paths:
                manifest.record_extract(provider, IngestManifest.logical_key(path), path)
        return ExtractStats(
